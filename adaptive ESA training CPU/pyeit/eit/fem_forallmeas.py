...
class Forward(object):
    ...
    def solve_eit(self, ex_mat=None, step=1, perm=None, parser=None):
        ...
        if ex_mat is None:
            ex_mat = cp.array(eit_scan_lines(self.ne, self.ne//2))
        else:
            ex_mat = cp.array(ex_mat)

        if type(step) is int:
            step_arr = step * cp.ones(self.ne)
        ...
        f = cp.asnumpy(f)
        f_el = f[:, self.n_pts:]
        #f_el = cp.asnumpy(f_el)
        volt_mat, ex_mat, new_ind = self.voltMeter(cp.asnumpy(ex_mat))
        #t3 = pt()
        V = self.substractRow(f_el, volt_mat, new_ind, 'v')
        J = self.substractRow(jac_i, volt_mat, new_ind, 'j').astype(np.float32)
        del(jac_i)
        #t4 = pt()
        #B = self.smearing(f, f_el, volt_mat, new_ind) # not needed for GREIT or AESA training
        B=0
        #t5 = pt()
        '''
        ...
        p = pde_result(jac=J[ind],
                       v=cp.asnumpy(V)[ind],
                       b_matrix=B)
     
        return p, meas[ind], ind.astype(np.int32)
    ...
    def solveAnomaly(self, ex_mat=None, step=1, perm=None, parser=None):
        ...
        print('subtract_row takes:', t4-t3)
        '''
        meas = np.concatenate((cp.asnumpy(ex_mat), volt_mat), axis=1)
    ...
    def findJac(self, ex_mat, perm0, ke, f, r_el):
        ...
        jac[:] = np.einsum('ijk, jkp, ljp->lij', cp.asnumpy(r_el[:, self.tri]), cp.asnumpy(ke), cp.asnumpy(f[:, self.tri]), optimize='optimal')
    ...
    def substractRow(self, f_el, volt_mat, new_ind, mode='original'):
        '''
        ...
        # perform subtraction
        #v_diff = f_el[new_ind, i] - f_el[new_ind, j] # original single line 
        
        if mode == 'v':
            v_diff = np.zeros(len(new_ind))
            def v_diff_calc(f_el, new_ind, i, j, k):
                return f_el[new_ind[k], i[k]] - f_el[new_ind[k], j[k]]
            init_time = datetime.now()
            v_diff[:] = Parallel(n_jobs=8)(delayed(v_diff_calc)(f_el, new_ind, i, j, k) for k in range(0,len(new_ind)))
            fin_time = datetime.now()
            #print("Subtract Volt:",fin_time-init_time)
            
        elif mode == 'j':
            v_diff = np.zeros((len(new_ind),len(f_el[0][0])))
            init_time = datetime.now()
            for k in range(0,len(new_ind)):
                v_diff[k] = f_el[new_ind[k], i[k]] - f_el[new_ind[k], j[k]]
            fin_time = datetime.now()
            print("Subtract Jac:",fin_time-init_time)
        elif mode == 'original':
            v_diff = f_el[new_ind, i] - f_el[new_ind, j]
        else:
            print("Invalid mode selected")
        # for k in range(0,len(new_ind)):
        #     v_diff2[k] = f_el[new_ind[k], i[k]] - f_el[new_ind[k], j[k]]
    ...
    def smearing(self, f, f_el, volt_mat, new_ind):
        ...
        b_matrix = np.empty((len(volt_mat), self.n_pts+self.ne)).astype(int)
        b_matrix[:] = (min_fel[:, None] < f[new_ind]) & (f[new_ind] <= max_fel[:, None])

        return b_matrix

    # def voltMeter_old(self, ex_mat, step_arr=None, parser=None):

    #     pair_mat = volt_mat(self.ne)
    #     srcsinkpairs = ex_mat.shape[0]

    #     pair_mat = np.tile(pair_mat, (int(ex_mat.shape[0]), 1, 1))
    #     pair_mat2 = cp.tile(cp.array(pair_mat), (int(ex_mat.shape[0]), 1, 1))
    #     pair_mat = pair_mat.reshape((pair_mat.shape[0] * pair_mat.shape[1], 2))
    #     pair_mat2 = pair_mat2.reshape((pair_mat2.shape[0] * pair_mat2.shape[1], 2))
    ...
    #     ind_fin = np.ones(len(ind_)) # My method, iterates through massive 2D vector line by line, ergo no memory allocation
    #     ind_fin2 = np.ones(len(ind_))
        
    #     def ind_finder(ind,nz,i):
    #         return np.sum(ind[i, None] == nz[0][None])
        
    #     init_time = datetime.now() # Multi core loop (same operation as single core loop below)
    #     ind_fin[:] = Parallel(n_jobs=8)(delayed(ind_finder)(ind_, nz2, i) for i in range(0,len(ind_)))
    #     ind_fin2[:] = Parallel(n_jobs=8)(delayed(ind_finder)(ind_, nz3, i) for i in range(0,len(ind_)))
    #     fin_time = datetime.now()
    #     print(fin_time-init_time)
        
    #     # init_time = datetime.now() # Single core loop
    #     # for i in range(0,len(ind_)):
    #     #     ind_fin[i] = np.sum(ind_[i, None] == nz2[0][None])
    #     #     ind_fin2[i] = np.sum(ind_[i, None] == nz3[0][None])
    #     # fin_time = datetime.now()
    #     # print(fin_time-init_time)
        
        
    #     # ind_fin = np.sum(ind_[:, None] == nz2[0][None], axis=1) # Original  method(makes massive 2D vector)
    #     # ind_fin2 = np.sum(ind_[:, None] == nz3[0][None], axis=1)

    #     ind_test = np.less((ind_fin + ind_fin2), 0.5 * np.ones(len(ind_fin))).astype(np.int32)
    ...
    def voltMeter(self, ex_mat):
        init_time=datetime.now()
        all_pairs = volt_mat(self.ne)
        pair_mat = []
        for ex_line in ex_mat: # Each ex_line is a current pair. Removing volt pairs which cross over with the current pair
            ...
            if len(pair_mat)==0:
                pair_mat = valid_pairs
            else:
                pair_mat = np.append(pair_mat,valid_pairs,axis=0)
        ind = np.zeros(len(pair_mat))
        pairs_per_current = int(len(pair_mat)/len(ex_mat))
        for i in range(0,len(ex_mat)):
            ind[pairs_per_current*i:pairs_per_current*(i+1)] = i
        ind = ind.astype(np.int32)
        
        fin_time = datetime.now()
        #print("Volt meter:", fin_time-init_time)
        
        return pair_mat, ex_mat[ind], ind

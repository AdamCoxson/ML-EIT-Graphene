...
def mesh_gen(n_el=20, num_per_el=3, start_pos='left', el_width=0.1, a=2.0, b=2.0, edge=0.1, mesh_params=None):
    ''' 
    ...
    p_fix1, el_pos = fix_electrodes_multiple(edgeX=edgeX, edgeY=edgeY, a=a, b=b, ppl=n_el,
                                                     el_width=el_width, num_per_el=num_per_el, start_pos=start_pos)
    #p_fix1 = np.array([[-1,1],[1,1],[1,-1],[-1,-1]])
    ...
    if do_plot == True:
        ...
        for i in range(n_el):
            ax.plot(el_pos[(i*num_per_el):(i*num_per_el)+num_per_el, 0], el_pos[(i*num_per_el):(i*num_per_el)+num_per_el, 1],  linestyle='-', lw=2.5, color = 'red')
...
def mesh(n_el=20, num_per_el=3, start_pos='left', el_width=0.1, a=2.0, b=2.0, edge=0.1, mesh_params=None, ref_perm=1.):
    # generate and extract meshing
    mesh_dict = mesh_gen(n_el=n_el, num_per_el=num_per_el, start_pos=start_pos,
                         el_width=el_width, a=a, b=b, edge=edge, mesh_params=mesh_params)
    p = mesh_dict['p']
    t = mesh_dict['t']
    del mesh_dict
    # create a constant permitivity array for reference
    perm = ref_perm*np.ones(t.shape[0], dtype=np.float)
...
#meshing; mesh(), mesh_gen(), fix_electrodes_multiple()

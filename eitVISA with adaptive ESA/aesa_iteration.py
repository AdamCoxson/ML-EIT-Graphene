...
def reconstruction_plot(ex_volt_mat, measured_voltages, mesh_obj=None, start_pos='left', ref_perm=0.03352, n_el=32, n_per_el=3, p=0.2, lamb=0.01, n_pix=64):
    
    #extended_ex_mat   = ex_volt_mat[:, :2]
    volt_mat = ex_volt_mat[:, 2:]
    ex_mat, ind = ex_mat_ind_finder(ex_volt_mat)
    if mesh_obj == None:
        mesh_obj = mesh(n_el=n_el, num_per_el=n_per_el, start_pos=start_pos, ref_perm=ref_perm) # Making an empty mesh
...
if __name__ == "__main__": ##################################################
    """ 
    The command if __name__ == "__main__": means this main only activates if the aesa_iteration.py is open at the time of run.
    This demonstration section shows how the A-ESA works and loops for simulated data, with a comparison to standard algorithms.
    """
    ...
    save_plots = False
    plot_set = 1
    demo_no = 2 # Demo_no=1 for a single iteration and plot. Demo_no=2 for many adaptive ESA loops and comparison to adj-adj method 
    ...
    ex_volt_mat, ex_mat, volt_mat, ind = initialise_ex_volt_mat(current_mode=current_mode, volt_mode=volt_mode,n_el=n_el, 
                                                                ex_mat_length=ex_mat_length, ex_spaced=True)
    #mesh_params = [0.045,2500,3.67,3.13]
    #mesh_params = [0.081,2500,9.55,3.14]
    mesh_params = [0.054, 2500, 10., 10.]
    #aesa = [0.80460672,38,0.361389917,41] # not that good # made for 20 electrodes
    #aesa = [0.69037305,-14,0.156208803,24] # decent, but doesn't quite cut it
    #aesa = [0.90,-10,0.5,10] # works okay
    #aesa = [0.99,13,0.1112,-22]  #pretty shit         # Trained on 64 electrodes
    #aesa = [0.916080914274607,20,0.445862118601088,-20]
    #aesa = [0.623921886,11,0.91789927,-15] # pretty shit as well
    ...
    if simulate_anomalies is True:
        ...
        mesh_new = train.set_perm(mesh_obj, anomaly=anomaly, background=1.0) # New mesh with anomalies
        ...
        plt.show()
    #reconstruction_plot(ex_volt_mat, voltages, mesh_obj, n_el=64, n_per_el=3, p=0.5, lamb=0.01, n_pix=64)     
    ...
    if demo_no == 1: # SINGLE ITERATION DEMO
        proposed_ex_volt_lines, ex_volt_meas, ex_mat, ind, reconstruction, total_map = aesa_prediction(mesh_obj,
                                        volt_mat=volt_mat, ex_mat=ex_mat, ind=ind, voltages=voltages,volt_mode=ESA_volt_mode, n_of_voltages=150,
                                        n_el=n_el, n_per_el=n_per_el,p=p,lamb=lamb,
    ...
    if demo_no == 2: # ADAPTIVE ESA LOOP AND ADJ-ADJ COMPARISON DEMO
        ...
        for i in range(1,prediction_loops+1):
            ...
            proposed_ex_volt_lines, ex_volt_meas, ex_mat, ind, reconstruction, total_map, counter = aesa_prediction(mesh_obj, 
                                            volt_mat=volt_mat, ex_mat=ex_mat, ind=ind, voltages=voltages, volt_mode=ESA_volt_mode, n_of_voltages=100,
            ...
            if i in (1, prediction_loops//4, prediction_loops//2, 3*prediction_loops//4, prediction_loops):
                ...
                if i in (1, prediction_loops//4,3*prediction_loops//4):
                    plt.close()
        reconstruction_plot(ex_volt_meas, voltages, mesh_obj, n_el=64, n_per_el=3, p=0.5, lamb=0.01, n_pix=128)  
        print("\nCreating Opp-Adj Comparison plots")           
        #for i in (10,15,20,25,32):
        #for i in (10,25,40,64):
        for i in (25,64):
            comp_ex_volt_mat, comp_ex_mat, comp_volt_mat, comp_ind = initialise_ex_volt_mat(current_mode='opp', volt_mode='adj', n_el=n_el, ex_mat_length=i)
            empty_mesh_f, empty_meas, empty_ind = fwd.solve_eit(volt_mat=comp_volt_mat, new_ind=comp_ind, ex_mat=comp_ex_mat)
            ...
            greit.setup(p=0.5, lamb=0.01, n=n_pix)
            h_mat = greit.H
...
"""
    Description of arrays and their formatting

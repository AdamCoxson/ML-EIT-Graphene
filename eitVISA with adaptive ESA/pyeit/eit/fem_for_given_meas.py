...
class Forward(object):
    ...
    def __init__(self, mesh, el_pos, ne, z=None):
        ...
        ref_el = 0
        while ref_el in self.el_pos: # This is broken and causes small anomalous low sigma spot in bottom left of plot.
            ref_el = ref_el + 1
        self.ref = ref_el
        ...
        if z is None:
            ...
            #self.z = 1. * cp.ones(self.ne)
            #self.z = 1e-8 * cp.ones(self.ne)
            self.z = 0.1*cp.ones(self.ne)
      
        else:
            self.z = z
    #@profile
    def solve_eit(self, volt_mat=None, new_ind=None, ex_mat=None, step=1, perm=None, parser=None, drv_a=1.0, drv_b=-1.0):
        """
        ...
        b = self._natural_boundary(ex_mat, drv_a, drv_b)
    ...
    def _natural_boundary(self, ex_mat, drv_a=1.0, drv_b=-1.0):
        """
        Notes
        -----
        ...
        b[drv_a_global_arr, row] = drv_a
        b[drv_b_global_arr, row] = drv_b
        # b[drv_a_global_arr, row] = 1.0e-4
        # b[drv_b_global_arr, row] = -1.0e-4
        # b[drv_b_global_arr, row] = 0
        # b[drv_a_global_arr, row] = 1.0
        # b[drv_b_global_arr, row] = -1.0
    ...
    def admittanceMatrixE(self):
        ...
        while (index_pointer[1:]==0).any():
            ...
            if loop_counter > 150:
                raise Exception('Exception: Stuck in FEM while loop')

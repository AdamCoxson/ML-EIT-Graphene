...
class OptimiseMeasurementAlgorithm(object):
    ...
    def getJacobian(self):
          ...
          #mesh_params = [0.08, 2000, 3.67, 3.13]
          mesh_params = [0.1,2000,0.45, 0.3]
...
n_samples = 100 # Default was 1000
n_el = 20
n_per_el=2
h0 = 0.054 # DONT FORGET TO CHANGE THE MESH PARAMS IN getJacobian() #!#!#!
a_mesh_coeff = 0.34
b_mesh_coeff = 1.89
contact_width = 2    # in percentage of side length
init_time = datetime.now()
opt = OptimiseMeasurementAlgorithm(n_samples=n_samples, ne=n_el, npix=64, n_per_el=2)
#test.optimise()
...
result = gp_minimize(opt.objective_function, bounds, x0=initial,
                    acq_func = "gp_hedge", n_calls=n_calls, n_random_starts=n_rand_calls, verbose=True, noise=1e-3, n_jobs=-1, model_queue_size=3)
...
filename = 'Bayesian_adaptiveESA_test'+'.csv'

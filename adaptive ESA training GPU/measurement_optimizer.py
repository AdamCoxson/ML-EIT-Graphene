...
def find_all_distances(rec, h_mat, el_coords, n_el, a = 2., npix = 64, cutoff = 0.8):
	...
	#print(el_coords)
	all_meas_pairs = volt_mat_all(n_el)
...
def findNextPair(fileJac, ne, anomaly=None, el_dist=None, meas=None, a=2., npix=64, pert=0.5, p_influence=-10., p_rec=10.):	
	...
	if meas is None:
		...
		A = np.arange(ne)
		B = (np.arange(ne) + el_dist) % ne
		ex_mat = np.stack((A, B), axis=1)
		...
		measurements = np.concatenate((ex_mat, volt_mat_all), axis=1)
	else:
		measurements = meas
	#print(measurements)
...
def voltMeterwStep(ne, ex_mat, step_arr=None, parser=None):
    ...
    '''        
    if step_arr is None:
        step_arr = 1 + np.arange((ex_mat.shape[0])) % (ne)
    elif type(step_arr) is int:
    	step_arr = step_arr * np.ones(ex_mat.shape[0]) % ne
    ...
    #M = np.dot(np.ones(ex_mat.shape[0])[:,None], A[None, :]) % self.ne
    #N = (M + step_arr[:, None]) % self.ne

    M = np.arange(ex_mat.shape[0] * ne) % ne
    N = (M.reshape((ex_mat.shape[0], ne)) + step_arr[:, None]) % ne
    pair_mat = np.stack((N.ravel(), M), axis=-1)

    #ind_new = np.arange(pair_mat.shape[0]) % ex_mat.shape[0]
    ind_new = np.arange(ex_mat.shape[0])        
    ind_new = np.tile(ind_new, (ne, 1)).T.ravel()
    #print('before indtest', ind_new[20:70])
    nz2 = np.where(pair_mat == drv_a[ind_new, None])
    nz3 = np.where(pair_mat == drv_b[ind_new, None])
    #print(ind_new)
    ...
    sort_index = np.argsort(ind_new)

    #print('after indtest', ind_new[20:70])
    #meas = np.concatenate((ex_mat[ind_new], pair_mat), axis=1)
    #print(meas[20:70])
...
def simulateMeasurements(fileJac, anomaly=0, measurements=None, v_meas=None, n_el=20, n_per_el=3, n_pix=64, a=2.):
	...
	if measurements is None:
		el_dist = np.random.randint(1, 20)
		ex_mat = (np.concatenate((np.arange(20)[None], (np.arange(20) + el_dist)[None])) % 20).T
		#print(ex_mat.shape)
		fem_all = Forward(mesh_obj, el_pos)
		measurements = fem_all.voltMeter(ex_mat)
		#ex_mat = mesurements[1]
		measurements = np.concatenate((measurements[1], measurements[0]), axis=1)
	...
	index = (np.sum(np.equal(measurements[:, None, :], meas[None, :, :]), axis=2) == 4)
	...
	if v_meas is None:
		...
		f2, raw, ind = fem.solve_eit(volt_mat=meas[ind, 2:], new_ind=new_ind[ind], ex_mat=meas[ind, :2], parser=None, perm=mesh_new['perm'].astype('f8'))
		v_meas = f2.v
		'''
		#plot
		fig = plt.figure(3)
